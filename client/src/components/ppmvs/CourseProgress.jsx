...
export default function CourseProgress() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('ppmvsProgress');
    if (saved) setProgress(JSON.parse(saved).percent || 0);
  }, []);

  const saveProgress = (percent) => {
    setProgress(percent);
    localStorage.setItem('ppmvsProgress', JSON.stringify({ percent, completed: [1, 2] }));
  };
  ...
  return (
    <div className="rounded-2xl border border-slate-700/50 bg-slate-900/50 p-6 shadow-xl">
      <h3 className="mb-4 text-lg font-bold text-white">Course Progress</h3>
          ...
          <div key={i} className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="h-2 w-full rounded-full bg-slate-700">
                <div className="h-2 rounded-full bg-gradient-to-r from-emerald-400 to-amber-400 transition-all" style={{ width: `${p}%` }} />
              </div>
            </div>
