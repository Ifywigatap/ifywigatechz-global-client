...
import { MODULES } from '../data/dataAnalyticsData.js';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900">
      <div className="container mx-auto px-6 py-12">
        <Link to="/data-analytics" className="mb-8 inline-flex items-center gap-2 rounded-xl bg-slate-800 px-4 py-2 text-slate-300 transition-all hover:bg-slate-700">
          ...
          <div className="lg:col-span-2">
            <div className="relative mb-8 aspect-video overflow-hidden rounded-2xl bg-slate-800 shadow-2xl">
              {module.videoId ? (
                ...
                <div className="flex h-full items-center justify-center bg-gradient-to-r from-blue-800 to-cyan-700">
            ...
            <div className="space-y-6">
              ...
              <div className="relative">
                <div className="rounded-2xl border border-slate-700 bg-slate-800/50 p-8 backdrop-blur-xl transition-all hover:border-cyan-400/30">
              ...
              <div className="flex gap-4">
                <button onClick={markComplete} className="flex-1 rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-500 px-8 py-4 font-bold text-white shadow-xl transition-all hover:scale-[1.02]" disabled={isCompleted}>
                  {isCompleted ? '✓ Module Complete!' : 'Mark Complete'}
                </button>
                <Link to="/data-analytics" className="flex-1 rounded-2xl border border-slate-700 bg-slate-800 px-8 py-4 text-center font-semibold text-slate-200 transition-all hover:bg-slate-700">
                  Back to Overview
                </Link>
              </div>
            </div>
            ...
            <div className="rounded-[2rem] border border-slate-700/50 bg-slate-900/70 p-6 shadow-2xl backdrop-blur-xl">
              ...
              <div className="space-y-3">
                {MODULES.slice(parseInt(module.id)).map((m) => (
                  <Link key={m.id} to={`/data-analytics/module/${m.id}`} className="group block rounded-3xl border border-slate-800/70 bg-slate-950/60 p-4 transition hover:border-cyan-400/40">
