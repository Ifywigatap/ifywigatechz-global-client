...
const TESTIMONIALS = [
  { text: 'Sarahs course took my designs from amateur to professional.', name: 'Chinedu Okeke, Freelance Designer', rating: 5 },
  { text: 'Best investment in my career. The Figma + Photoshop combo modules are gold.', name: 'Fatima Ibrahim', rating: 5 },
  { text: 'Real-world projects helped me build a killer portfolio.', name: 'Amina Yusuf, UI Designer', rating: 5 },
];

export default function GraphicTestimonialSlider() {
    ...
    <div className="relative rounded-2xl border border-slate-700/50 bg-slate-900/70 p-8 backdrop-blur-xl shadow-2xl">
      <div className="mb-6 text-center">
        <div className="mx-auto mb-4 inline-flex h-3 w-20 items-center rounded-full bg-slate-600">
          <div className="h-3 flex-1 rounded-full bg-gradient-to-r from-purple-400 to-pink-400 animate-pulse" />
        </div>
        <div className="flex items-center justify-center gap-2">
          {Array.from({length: 5}).map((_, i) => (
            <div key={i} className={`h-1.5 w-1.5 rounded-full ${i < testimonial.rating ? 'bg-purple-400' : 'bg-slate-600'}`} />
          ))}
        </div>
      </div>
