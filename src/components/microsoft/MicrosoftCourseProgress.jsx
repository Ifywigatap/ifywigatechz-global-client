...
export default function MicrosoftCourseProgress() {
  const [progress, setProgress] = useState(25);

  useEffect(() => {
    const saved = localStorage.getItem('microsoftProgress');
    if (saved) setProgress(JSON.parse(saved).percent || 0);
  }, []);

  const saveProgress = (percent) => {
    setProgress(percent);
    localStorage.setItem('microsoftProgress', JSON.stringify({ percent, completed: [1, 2] }));
  };
  ...
  return (
    <div className="rounded-2xl border border-slate-700/50 bg-slate-900/50 p-6 shadow-xl">
      <h3 className="mb-4 text-lg font-bold text-white">Microsoft Course Progress</h3>
      <div className="space-y-3">
        {modules.map((p, i) => (
          <div key={i} className="flex items-center gap-3">
            <div className="flex h-2.5 w-2.5 items-center justify-center rounded-full bg-slate-600 font-bold text-xs">
              {p > 0 ? <CheckIcon className="h-3 w-3 text-blue-400" /> : i + 1}
            </div>
            <div className="flex-1">
              <div className="h-2 w-full rounded-full bg-slate-700">
                <div className="h-2 rounded-full bg-gradient-to-r from-blue-400 to-blue-600 transition-all" style={{ width: `${p}%` }} />
              </div>
            </div>
